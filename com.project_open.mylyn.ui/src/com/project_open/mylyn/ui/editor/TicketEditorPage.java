...
public class TicketEditorPage extends TaskFormPage {
	...
	public TicketEditorPage(TaskEditor editor, String title) {
		...
		ProjectOpenRepositoryConnector connector = ProjectOpenCorePlugin
	...
	private void downloadTicketAndRefresh() {
		Job job = new Job(NLS.bind("Retrieving ticket {0}...", getTask()
				.getTaskId())) {
			@Override
			protected IStatus run(IProgressMonitor monitor) {
				...
				try {
					ticket = client.getTicket(
							Integer.valueOf(getTask().getTaskId()).intValue(),
							monitor);
				} catch (Exception e) {
					return new Status(IStatus.ERROR,
							ProjectOpenUiPlugin.PLUGIN_ID, e.getMessage());
				}
				return new Status(IStatus.OK, ProjectOpenUiPlugin.PLUGIN_ID,
						null);
			}
		};
